...
const Sidebar = () => {
        ...
        if (res.data) {
          ...
          if (res.data.logo) {
             ...
             if (logoPath.startsWith('http')) {
               ...
               const hasUploads = logoPath.startsWith('uploads/');
               const normalized = hasUploads ? `/${logoPath}` : `/uploads/${logoPath}`;
               setLogo(normalized);
             }
          }
        }
  ...
  return (
    <div className="w-64 bg-sidebar h-screen shadow-xl flex flex-col fixed overflow-y-auto z-20 text-black font-sans">
      {/* Header / Logo */}
        ...
        {links.map((link: any) => {
          if (link.isGroup) {
            return (
              <div key={link.label}>
                <button
                  onClick={link.toggle}
                  className="w-full flex items-center justify-between px-4 py-3 text-sm font-medium rounded-lg text-black hover:text-black hover:bg-gray-100 transition-colors"
                >
                    ...
                    {link.children.map((child: any) => {
                       ...
                       return (
                        <Link
                          ...
                          to={child.path}
                          className={clsx(
                            'flex items-center px-4 py-2 text-sm font-medium rounded-lg transition-all ml-4 border-l border-gray-300 pl-4',
                            isChildActive
          ...
          return (
            ...
            >
              ...
              <Icon className={clsx("w-5 h-5 mr-3 transition-colors", isActive ? "text-white" : "group-hover:text-black")} />
              {link.label}
            </Link>
          );
        })}
